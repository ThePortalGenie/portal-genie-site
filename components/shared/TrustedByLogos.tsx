...
const logoSize: Record<TrustedByVariant, string> = {
  default: "max-h-12 md:max-h-14",
  compact: "max-h-10 md:max-h-12",
  hero: "max-h-[3.75rem] md:max-h-[4.375rem]",
};
...
function LogoCard({
  ...
  return (
    ...
    >
      <div
        className={`group flex w-full items-center justify-center rounded-card border border-muted/20 shadow-[0_2px_8px_-4px_rgba(17,33,54,0.06)] transition-all duration-300 hover:-translate-y-0.5 hover:border-muted/35 hover:shadow-[0_8px_24px_-8px_rgba(17,33,54,0.12)] motion-reduce:transition-none motion-reduce:hover:translate-y-0 ${cardBackground} ${cardHeightClass} ${cardPaddingClass}`.trim()}
      >
        <Image
          src={logo.image}
          alt={logo.alt}
          ...
          height={72}
          loading="lazy"
          className={`w-auto max-w-full object-contain opacity-80 transition-opacity duration-300 group-hover:opacity-100 ${logoSizeClass}`.trim()}
        />
      </div>
    </li>
  );
  ...
  return (
    ...
    >
      <Container>
        ...
        <div ref={ref}>
          <ul
            className={`grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 md:gap-5 lg:grid-cols-5 lg:gap-6 xl:grid-cols-6 ${gridSpacing[variant]}`.trim()}
          >
